...
class TensorConverter(PipelineComponent):
    ...
    def execute(self) -> None:
        ...
        # [FIX] Load correct target files generated by LabelGenerator (from ArtifactsConfig)
        ...
        if icd_targets_path.exists():
            ...
            # [FIX] Rename 'target_icd' to 'y' to match HDF5/Model expectation
            # icd_df has [ICUSTAY_ID, target_icd]
        ...
        if vent_targets_path.exists():
            ...
            # [FIX] Rename 'target_vent' to 'y_vent'
        ...
        # [FIX] Filter using "SUBJECT_ID" not "ICUSTAY_ID"
    ...
    def _process_split(self, split_df: pl.DataFrame, split_name: str, num_cols: List[str], cat_cols: List[str], cache_dir: Path, prefix: str, max_cands: int):
        ...
        # Fill Targets
        # Prioritize 'y_vent' from targets, fallback to 'VENT' from raw
        ...
        # [FIX] Transpose X_num to (Batch, Channel, Time) for PyTorch
        ...
        with h5py.File(out_path, "a") as f:
            ...
            if "y" in grouped.columns:
                try:
                    # 'y' can be a list of lists (multi-hot) or list of ints (single label)
                    # We cast to numpy. If it's ragged (object), we skip or warn.
                    y_list = grouped["y"].to_list()
                    icd_targets = np.array(y_list)
                    
                    # If object, try to stack (if dense)
                    if icd_targets.dtype == object:
                        try:
                            icd_targets = np.vstack(y_list).astype(np.float32)
                        except:
                            self.observer.log("WARNING", "ICD targets are ragged. Skipping 'y' dataset creation.")
                            icd_targets = None
                    else:
                        icd_targets = icd_targets.astype(np.float32)
                    
                    if icd_targets is not None:
