    ...
    def execute(self) -> None:
        ...
        def sent_tokenize_rules(text: str) -> List[str]:
            ...
            for segment in segments:
                ...
                for h in headers:
                    try:
                        ...
                        if prefix: new_segments.append(prefix)
                        new_segments.append(h)
                        segment = rest
                    except ValueError: continue
                if segment: new_segments.append(segment)
            ...
            for segment in segments:
                ...
                for ss in parts[1:]: new_segments += ["_____", ss]
            segments = new_segments
            ...
            for segment in segments:
                parts = segment.split("\n-----\n")
                new_segments.append(parts[0])
                for ss in parts[1:]: new_segments += ["-----", ss]
            segments = new_segments
            ...
            if segments:
                for segment in segments:
                    ...
                    if seg: new_segments.append(seg)
            segments = new_segments
            ...
            for i, seg in enumerate(segments):
                if i > 0 and "\n" not in seg and is_title(segments[i - 1]) and (i == N - 1 or is_title(segments[i + 1])):
                    if new_segments: new_segments[-1] = segments[i - 1] + " " + seg
