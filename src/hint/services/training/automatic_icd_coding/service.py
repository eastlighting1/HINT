...
class ICDService(BaseDomainService):
    ...
    def execute(self) -> None:
        ...
        # Calculate Class Frequency for Loss (using Training set)
        try:
        ...
        # Loop per Model: Train -> Val -> Test -> Next Model
        for model_name in models_to_run:
            ...
            try:
                ...
                # 4. Run Training (includes Validation)
                ...
                best_state = self.registry.load_model(entity_name, version="best")
                ...
                self.observer.track_metric(f"{model_name}_test_acc", test_metrics["accuracy"])
                self.observer.track_metric(f"{model_name}_test_f1", test_metrics["f1_macro"])
