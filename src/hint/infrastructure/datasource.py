...
class ICDDataset(Dataset):
    ...
    def __init__(self, df, feats, label_col, list_col, cand_col="candidate_indices"):
        ...
        if cand_col in df.columns: self.cand = df[cand_col].tolist()
        else: self.cand = [[int(lbl)] for lbl in self.y]
...
def collate_tensor_batch(batch: List[TensorBatch]) -> TensorBatch:
    ...
    tb = TensorBatch(x_num=x_num, x_cat=x_cat, y=y, ids=ids, mask=mask, x_icd=x_icd, input_ids=input_ids, attention_mask=attention_mask, candidates=candidates)
    if delta is not None: setattr(tb, "delta", delta)
    return tb
...
class ParquetSource(StreamingSource):
    ...
    def __init__(self, file_path: Union[str, Path]):
        ...
        if not self.file_path.exists():
            if not self.file_path.is_absolute(): self.file_path = Path.cwd() / self.file_path
            if not self.file_path.exists(): raise DataError(f"Parquet file not found at {self.file_path}")
        self._df: Optional[pl.DataFrame] = None
        
    def load(self) -> pl.DataFrame:
        try:
            if self._df is None: self._df = pl.read_parquet(self.file_path)
            return self._df
        except Exception as e: raise DataError(f"Failed to load parquet file: {e}")

    def __iter__(self) -> Generator[Dict[str, Any], None, None]:
        try:
            if self._df is None: self._df = pl.read_parquet(self.file_path)
            for row in self._df.iter_rows(named=True): yield row
        except Exception as e: raise DataError(f"Failed to stream data: {e}")

    def __len__(self) -> int:
        try: return pl.scan_parquet(self.file_path).select(pl.len()).collect().item()
        except Exception as e: raise DataError(f"Failed to get length of parquet file: {e}")
...
class HDF5StreamingSource(Dataset):
    ...
    def __init__(self, h5_path: Path, seq_len: int = 0, label_key: str = "y"):
        ...
        if not self.h5_path.exists(): raise DataError(f"HDF5 file not found at {self.h5_path}")
        try:
            with h5py.File(self.h5_path, "r") as f:
                if "X_num" not in f: raise DataError(f"HDF5 missing required dataset X_num in {self.h5_path}")
                self._len = len(f["X_num"])
        except Exception as e: raise DataError(f"Failed to initialize HDF5 source: {e}")
    ...
    def __getitem__(self, idx: int) -> TensorBatch:
        ...
        x_cat = None
        if "X_cat" in self.h5_file: x_cat = torch.from_numpy(self.h5_file["X_cat"][idx]).long()
        
        y_val = self.h5_file[self.label_key][idx]
        
        # [FIX] Handle time-series targets for sequence-to-one models
        # If the target is a sequence (e.g. 120 hours), take the LAST value as the classification label.
        # This matches GFINet_CNN which pools the last timestep.
        if y_val.ndim > 0:
            y_val = y_val[-1]
            
        # [FIX] Force LongTensor for classification targets (needed for F.one_hot)
        y = torch.tensor(int(y_val), dtype=torch.long)
        
        sid = -1
        if "sid" in self.h5_file: sid = int(self.h5_file["sid"][idx])
        sid_tensor = torch.tensor(sid, dtype=torch.long)
        
        x_icd = None
        if "X_icd" in self.h5_file: x_icd = torch.from_numpy(self.h5_file["X_icd"][idx]).float()
        ...
        tb = TensorBatch(x_num=x_num, x_cat=x_cat, y=y, ids=sid_tensor, mask=mask, x_icd=x_icd, input_ids=input_ids, attention_mask=attention_mask, candidates=candidates)
    ...
    def get_real_vocab_sizes(self) -> List[int]:
        ...
        if self.h5_file is None:
            ...
            should_close = True
        try:
            if "X_cat" not in self.h5_file: return []
            data = self.h5_file["X_cat"][:]
            if data.size == 0: return []
            if data.ndim == 2: max_indices = np.max(data, axis=0)
            else: max_indices = np.max(data, axis=(0, 2))
